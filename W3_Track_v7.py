# ==============================================================================
# AGV COMBINED SYSTEM v7.0 — The Ultimate Veto Synergy
# ==============================================================================
#
# CRITICAL FIXES APPLIED:
#   1. Unchoked ORB: Ratio back to 0.80, Matches back to 5. Symbols are highly 
#      sensitive and will trigger flawlessly again.
#   2. The Hard Veto: If ORB >= 3 matches, Arrow Detector is completely skipped. 
#      This stops the Blue QR Code from triggering false Left/Right turns.
#   3. The Color Defender: If a bright blue Arrow accidentally gets 3 ORB matches 
#      (hallucinating a BUTTON), the Color Defender forces matches to 0, dropping 
#      the Hard Veto and letting the Arrow trigger perfectly.
#
# ==============================================================================
...
def action_arrow_turn(direction):
    ...
    while True:
        if time.time() - start_time > 1.5:
            print("    -> [FAILSAFE] Arrow Turn Timeout! Aborting.")
            break
        ...
        if len(cnts) > 0:
            ...
            if cv2.contourArea(c) > 250:
                ...
                cx = int(x + (w / 2))
                if 100 < cx < 220:
                    print(f"    -> Line acquired! Stopping turn.")
                    break
        time.sleep(0.005)
    motor_stop()
...
# --- THE ORB UNCHOKE ---
ORB_RATIO_TEST = 0.85   # Forgiving to motion blur
MIN_MATCH_COUNT = 4      # Easy to trigger
...
ARROW_STABLE_FRAMES  = 3 # Fast snapping arrows
...
def run_orb(vision_roi_bgr, vision_roi_hsv):
    ...
    if des_live is None or len(kp_live) < 5: return None, 0, "KP:<5 (blind)", None
    ...
    # --- THE COLOR DEFENDER ---
    # Fast check the middle of the camera feed for bright colors.
    h_roi, w_roi = vision_roi_hsv.shape[:2]
    cy, cx = h_roi // 2, w_roi // 2
    center_sat = np.median(vision_roi_hsv[cy-20:cy+20, cx-20:cx+20, 1])

    # If it sees bright color (Arrow) but thinks it's a Black/White symbol, it's hallucinating!
    # Zero out the matches instantly to protect the Arrow Detector.
    if center_sat > 80:
        if best_sym in ["QR_CODE", "FINGERPRINT", "BUTTON"]:
            max_inliers = 0
            best_sym = None
            best_H = None
...
def detect_arrows(bgr_roi, display_frame, roi_y_offset=10):
    ...
    for cnt in cnts:
        ...
        if area < 800 or area > 12000: continue

        x, y, w, h = cv2.boundingRect(cnt)
        if h == 0 or w == 0 or w > 200 or h > 200: continue
        if y < 25: continue 
        ...
        if solidity < 0.40 or solidity > 0.85: continue
        ...
        cv2.putText(display_frame, label, (x, fy - 6), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (255, 100, 50), 2)
        tally[label] = tally.get(label, 0) + 1

    recycle_suspected = False
    detected_dir = None

    if len(found_boxes) >= 2:
...
def line_thread():
    ...
    while True:
        ...
        with shared_lock:
            ...
            orb_inliers = shared.get("orb_inliers", 0)
        ...
        if now > shortcut_lockout_until:
            ...
            cnts_red    = get_solid_shortcut_contours(red_mask)
            cnts_yellow = get_solid_shortcut_contours(yellow_mask)
        else:
            cnts_red = []
            cnts_yellow = []
        ...
        if len(cnts_red) > 0:
            ...
            if now < shortcut_lockout_until:
                ...
                tracking_label = "BLACK (ORB BRAKING)"
...
def vision_thread():
    ...
    while True:
        ...
        if in_action or now < cooldown:
            ...
            # --- THE HARD VETO ---
            # If ORB is tracking a complex symbol (QR/Biohazard), completely skip the Arrow Detector!
            arrow_veto = False
            if max_inliers >= 3:
                arrow_veto = True

            if not on_shortcut and orb_sym is None and not arrow_veto:
        ...
        cv2.rectangle(vision_display, (CROP_X, CROP_Y), (CROP_X + CROP_SIZE, CROP_Y + CROP_SIZE), (0, 165, 255), 1)   
...
def build_main_display():
    ...
    cv2.putText(dashboard, "AGV MAIN STATUS v11.9",
...
<html>
<head>
    <title>AGV Dashboard v11.9</title>
...
<body>
    <h1>AGV Dashboard v11.9</h1>
    <div class="fix-note">
      <b>v11.9 Synergy Update:</b> Unchoked ORB (Ratio 0.80, Matches 5) so QR Code gets high matches again. Hard Veto strictly blocks Arrow Detector when QR is active. Color Defender protects real Arrows from being vetoed!
    </div>
    <div class="container">
...
if __name__ == "__main__":
    ...
    try:
        print("\n" + "=" * 55)
        print("  AGV v11.9 Flask Server RUNNING")
