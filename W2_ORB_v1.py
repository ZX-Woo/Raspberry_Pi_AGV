...
print("Camera locked! Running Hybrid Mode (ORB for Textures, Geometry for Shapes).")

kernel = np.ones((5, 5), np.uint8)

# ==============================================================================
# 2. ORB MEMORY SETUP (ONLY for inconsistent textures like FP and QR)
# ==============================================================================
...
load_dir = os.path.expanduser("~/Downloads/symbols")

# Load Fingerprint
try:
    path_fp = os.path.join(load_dir, 'fingerprint_ref.jpg')
    ref_fp = cv2.imread(path_fp, cv2.IMREAD_GRAYSCALE)
    if ref_fp is None: raise FileNotFoundError
    kp_fp, des_fp = orb.detectAndCompute(ref_fp, None)
    print("Fingerprint ORB reference loaded successfully!")
except FileNotFoundError:
    print(f"WARNING: 'fingerprint_ref.jpg' not found in {load_dir}.")
    des_fp = None

# Load QR Code (Optional ORB boost, fallback to geometry if it fails)
try:
    path_qr = os.path.join(load_dir, 'qr_ref.jpg')
    ref_qr = cv2.imread(path_qr, cv2.IMREAD_GRAYSCALE)
    if ref_qr is None: raise FileNotFoundError
    kp_qr, des_qr = orb.detectAndCompute(ref_qr, None)
    print("QR Code ORB reference loaded successfully!")
except FileNotFoundError:
    print(f"WARNING: 'qr_ref.jpg' not found in {load_dir}.")
    des_qr = None


# ==============================================================================
# 3. THE PURE GEOMETRY MATH (Your exact flawless logic tree)
# ==============================================================================
...
try:
    while True:
        ...
        # --- 1. ORB TEXTURE MATCHING (Solves the Fingerprint Inconsistency) ---
        ...
        if des_frame is not None:
            # Check Fingerprint
            if des_fp is not None:
                matches = bf.match(des_fp, des_frame)
                good = [m for m in matches if m.distance < 60] 
                m_fp = len(good)
                if m_fp > 5: symbol_detected = "FINGERPRINT"
            
            # Check QR Code (Optional ORB boost)
            if des_qr is not None and not symbol_detected:
                matches = bf.match(des_qr, des_frame)
                good = [m for m in matches if m.distance < 60] 
                m_qr = len(good)
                if m_qr > 12: symbol_detected = "QR CODE"

        # Display ORB stats
        cv2.putText(frame, f"ORB Matches -> FP:{m_fp} | QR:{m_qr}", (10, 220), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.45, (255, 0, 255), 2)

        # --- 2. YOUR EXACT COMBO LOGIC (For all other shapes) ---
        # Only run the heavy geometry math if ORB didn't find the fingerprint
        if not symbol_detected:
            tally, bw_screen = detect_shapes(frame, frame)
            
            blue_circle_count = tally.get("3/4 Circle", 0)
            ...
            elif qr_square_count >= 3 or blue_circle_count >= 3: symbol_detected = "QR CODE" # Fallback if ORB missed it
            elif arrow_count >= 3: symbol_detected = "RECYCLE" # Updated to 3 arrows per your request!
            elif arrow_detected: symbol_detected = f"ARROW {arrow_detected.upper()}"
            
        else:
            # If ORB found a match, create a blank mask so the screen doesn't crash
            bw_screen = np.zeros((240, 320), dtype=np.uint8)
