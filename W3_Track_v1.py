# ==============================================================================
# W3_Track_v1 — 30 FPS Parallel Circuit
# ==============================================================================
#
...
#   - PID Sensitivity Fix: Raised max_speed to 90 so outer wheels can speed up!
#   - ORB_MIN_FLOOR set to 15 to completely eliminate false-positive symbols.
#   - Arrow detection uses Otsu's Luma Thresholding + Black Track Eraser.
#   - Real-World Arrow Fix: Removed vertex counting, increased blur, relaxed area_ratio.
#   - Ghost Queue Fix: Vision pauses and wipes memory during cooldowns.
#   - Flipped Arrow Logic: Left/Right and Up/Down corrected for camera mount.
#   - Shortcut Memory: Remembers entry side to smoothly exit color tracks.
...
# ==============================================================================
# 6. VISION MATH: ORB & ARROW DETECTION
# ==============================================================================
...
def detect_arrows(bgr_frame, display_frame):
    ...
    # 1. Convert to YUV to extract ONLY the brightness (Luma/Y channel)
    yuv = cv2.cvtColor(bgr_frame, cv2.COLOR_BGR2YUV)
    y_channel = yuv[:, :, 0]
    
    # 2. Blur heavily to smooth out pixelated edges and camera noise
    blurred_y = cv2.GaussianBlur(y_channel, (7, 7), 0)

    # 3. Otsu's Thresholding (Adapts to room lighting)
    _, cleaned = cv2.threshold(blurred_y, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)

    # 4. CRITICAL FIX 1: Erase the black track so it isn't confused for an arrow
    ...
    for cnt in cnts:
        ...
        # Aspect Ratio (Ignores long strips of stray black tape)
        aspect_ratio = float(w) / h
        if aspect_ratio < 0.4 or aspect_ratio > 2.5:
            continue 

        # SHAPE FILTER: Increased to 0.65 to allow for thicker arrows to pass through
        # No vertex/corner counting here anymore!
        ...
        # Camera Mount Flipper (Flips Left/Right and Up/Down)
...
def line_thread():
    ...
    while True:
        ...
        roi = bgr[260:360, 0:640] 
        blackline = cv2.inRange(roi, (0, 0, 0), (100, 100, 100))
        kernel = np.ones((5, 5), np.uint8)
        blackline = cv2.erode(blackline, kernel, iterations=2)

        target_mask = None
        if TARGET_FLOOR_COLOR != "BLACK":
            hsv_roi = cv2.cvtColor(roi, cv2.COLOR_BGR2HSV)
            if TARGET_FLOOR_COLOR == "RED":
                m1 = cv2.inRange(hsv_roi, (0,   100, 100), (10,  255, 255))
                m2 = cv2.inRange(hsv_roi, (160, 100, 100), (180, 255, 255))
                target_mask = cv2.bitwise_or(m1, m2)
            elif TARGET_FLOOR_COLOR == "YELLOW":
                target_mask = cv2.inRange(hsv_roi, (15, 100, 100), (35, 255, 255))
            elif TARGET_FLOOR_COLOR == "GREEN":
                target_mask = cv2.inRange(hsv_roi, (35, 80, 80),  (85, 255, 255))
            elif TARGET_FLOOR_COLOR == "BLUE":
                target_mask = cv2.inRange(hsv_roi, (85, 80, 80),  (130, 255, 255))

        active_mask    = None
        tracking_label = "BLACK"

        if target_mask is not None:
            clean_target = cv2.erode(target_mask, np.ones((5, 5), np.uint8), iterations=2)
            ...
            if len(contours_target) > 0:
                ...
                tracking_label = TARGET_FLOOR_COLOR   
                ...
                if not on_shortcut:
                    ...
                    entry_error = int(x_t + (w_t / 2)) - 160
                    shortcut_dir = "RIGHT" if entry_error >= 0 else "LEFT"
                    on_shortcut = True
                    print(f"[LINE] ENTERED SHORTCUT: Remembering to turn {shortcut_dir} later.")
        ...
        if active_mask is None:
            ...
            if on_shortcut:
                ...
                if shortcut_lost_frames > 2: 
                    print(f"[LINE] EXITED SHORTCUT: Forcing {shortcut_dir} turn to rejoin main track.")
        ...
        if len(contours) == 0:
            ...
            with shared_lock: shared["line_display"] = display_bgr
        ...
        cv2.line(display_bgr, (160, 240), (int(x + w / 2), 160 + int(y)), (255, 0, 0), 3)
        
        memory_txt = f" | Mem: {shortcut_dir}" if on_shortcut else ""
        cv2.putText(display_bgr, f"Tracking: {tracking_label}{memory_txt}",
                    (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 255, 255), 1)
        cv2.putText(display_bgr, f"err:{error}  pid:{pid:.1f}",
...
def vision_thread():
    ...
    while True:
        ...
        # =======================================================================
        # DETECTION HIERARCHY & GHOST QUEUE PREVENTION
        # =======================================================================
        now = time.time()
        with shared_lock:
            cooldown = shared["cooldown_until"]
...
if __name__ == "__main__":
    ...
    try:
        print("[MAIN] GUI Loop started. Displaying 3 windows.")
        while True:
            with shared_lock:
                if not shared["running"]: break
            ...
            if v_disp is not None: cv2.imshow("1. AGV Vision (Normal Display + Results)", v_disp)
            if m_disp is not None: cv2.imshow("2. Vision Mask", m_disp)
            if l_disp is not None: cv2.imshow("3. Line Trend", l_disp)
